...
parser = ArgumentParser(
    ...
    description = "Runs a RISC-V simulation executable on ISA tests - ELF files taken from <repo>/Tests/isa and its sub-directories. Runs it only on those ELF files that are relevant to architecture <arch>. For each ELF file FOO, saves simulation output in <logs_path>/FOO.log.", 
    epilog = """For Example: `$ <this_prog>  .exe_HW_sim  ~somebody/GitHub/Toooba ./Logs RV32IMU -v -p 4` will run the verilator simulation executable on the following RISC-V ISA tests:\n\t~somebody/GitHub/Tests/isa/rv32ui-p*\n\t~somebody/GitHub/Tests/isa/rv32mi-p*\n\t~somebody/GitHub/Tests/isa/rv32um-p*\nwhich are relevant for architecture RV32IMU, and will leave a transcript of each test's simulation output in files like `./Logs/rv32ui-p-add.log`. Each log will contain an instruction trace (because of the '-v' arg). It will use 4 processes in parallel to run the regressions. (creating temporary working directories worker_0, ..., worker_3). \n\nFor more details, see $(REPO)/Tests/Run_regression.py""", \
    add_help = True, \
...
parser.add_argument("arch", type=str, help="RV Architecture to run tests against - e.g:`RV64ACDFIMSU` - Required")
...
parser.add_argument("--no_store_logs", action="store_false", help="Write logfiles to tempdir and delete when finished to reduce storage overhead of run - if this argument is specified, only failed tests will be written to 'logs_path'")
...
def main (argv = None):
    ...
    if (not os.path.exists (elfs_path)):
        error(f"ERROR: ELFs directory ({repo}) does not exist?\n")
    ...
    arch_string = extract_arch_string (args.arch)
    if (arch_string == None):
        error("ERROR: no architecture specified?\n")
    ...
    args_dict['keep_logs'] = args.no_store_logs
...
def do_isa_test (args_dict, full_filename):
    ...
    with open(log_filename, 'r') as fd:
        passed = fd.read().find("PASS") != -1

    if (not args_dict['keep_logs']) and (not passed):
        message = message + (f"Test failed - copying log to {backup_log_filename}")
    ...
    if (not args_dict['keep_logs']):
        os.remove(os.path.abspath(log_filename))
...
def run_command (command, log_fd):
    log_fd.write(f"Running: {command}\n")
